...
function DocumentCard({ document, index }: { document: (typeof schoolContent.mandatoryDisclosure.documents)[number]; index: number }) {
  const content = <><div className="relative flex flex-1 flex-col items-center justify-center gap-3 px-2 pt-5 text-center sm:gap-5 sm:px-5 sm:pt-7"><div className="grid size-10 place-items-center rounded-xl bg-[#f8eedc] text-[#c94b35] shadow-sm sm:size-14 sm:rounded-2xl"><FileText size={23} strokeWidth={1.8} className="sm:hidden" /><FileText size={30} strokeWidth={1.8} className="hidden sm:block" /></div><h3 className="max-w-[17rem] font-display text-[1.05rem] font-bold leading-[.98] text-[#202337] sm:text-2xl">{document.title}</h3></div><div className="flex min-h-10 items-center justify-center gap-1 bg-[#202337] px-1 py-2 text-center text-[10px] font-bold leading-tight text-[#f8eedc] sm:min-h-0 sm:gap-2 sm:px-4 sm:py-3 sm:text-xs">{document.file ? <>Download <Download size={13} /></> : <>Add PDF <FileText size={12} /></>}</div></>;
  return document.file ? <a href={document.file} download className="group flex min-h-[205px] flex-col overflow-hidden rounded-[1.15rem] border border-[#202337]/10 bg-[#e7f1ed] shadow-sm transition-transform hover:-translate-y-1 sm:min-h-[235px] sm:rounded-[1.7rem]" data-testid={`link-disclosure-document-${index}`}>{content}</a> : <div className="flex min-h-[205px] flex-col overflow-hidden rounded-[1.15rem] border border-dashed border-[#202337]/20 bg-[#e7f1ed] sm:min-h-[235px] sm:rounded-[1.7rem]" data-testid={`card-disclosure-document-${index}`}>{content}</div>;
}
