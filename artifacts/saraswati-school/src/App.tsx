      ...
      <section className="bg-[#e3b45b] px-5 py-14 text-[#202337]">
        <div className="page-wrap flex flex-col justify-between gap-7 md:flex-row md:items-center">
          <div>
            ...
            <h2 className="mt-3 font-display text-4xl font-bold leading-none md:text-5xl">A question is just one call away.</h2>
            <p className="mt-3 max-w-xl text-sm leading-6 text-[#202337]/70">Call during office hours for admissions, campus visits, fees, or any help your family needs.</p>
          </div>
...
function Admissions() {
  ...
  const sendEnquiry = (event: FormEvent<HTMLFormElement>) => {
    ...
    const subject = `Admission enquiry — ${form.student}`;
    const body = [
    ...
    window.location.assign(`mailto:lewaedusaraswati@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`);
    setSubmitted(true);
  };
  return <>
    ...
    <section className="bg-[#3b7f7c] px-5 py-20 text-[#f8eedc] md:py-28"><div className="page-wrap grid gap-14 lg:grid-cols-[.72fr_1.28fr]"><div><p className="font-mono-school text-[10px] uppercase tracking-[.18em] text-[#e3b45b]">Your first step</p><h2 className="mt-4 font-display text-5xl font-bold leading-[.9] md:text-7xl">Tell us a little<br />about your family.</h2><p className="mt-6 max-w-sm text-sm leading-6 text-[#f8eedc]/65">Your enquiry opens a ready-to-send email addressed to the school office. No details are sent to any third party.</p></div><div className="rounded-[2rem] bg-[#f8eedc] p-6 text-[#202337] md:p-9">{submitted ? <div className="flex min-h-[410px] flex-col items-center justify-center text-center"><div className="grid size-16 place-items-center rounded-full bg-[#3b7f7c] text-[#f8eedc]"><Check size={30} /></div><h2 className="mt-6 font-display text-5xl font-bold">Your email is ready.</h2><p className="mt-4 max-w-sm text-sm leading-6 text-[#202337]/60">Please press send in your email app to share the enquiry with the school office.</p><button type="button" onClick={() => { setSubmitted(false); setForm({ student: '', parent: '', email: '', phone: '', className: '', message: '' }); }} className="mt-7 font-mono-school text-[10px] font-bold uppercase tracking-[.16em] text-[#c94b35] underline underline-offset-4" data-testid="button-submit-another">Send another enquiry</button></div> : <form onSubmit={sendEnquiry} className="grid gap-5"><input type="text" value={honeypot} onChange={(event) => setHoneypot(event.target.value)} aria-hidden="true" tabIndex={-1} autoComplete="off" className="hidden" /><div className="grid gap-5 sm:grid-cols-2"><label className="grid gap-2 text-xs font-bold">Student name<input required value={form.student} onChange={update('student')} placeholder="Child's full name" className="school-input" data-testid="input-student-name" /></label><label className="grid gap-2 text-xs font-bold">Parent / guardian name<input required value={form.parent} onChange={update('parent')} placeholder="Your full name" className="school-input" data-testid="input-parent-name" /></label></div><div className="grid gap-5 sm:grid-cols-2"><label className="grid gap-2 text-xs font-bold">Parent contact<input required type="tel" value={form.phone} onChange={update('phone')} placeholder="+91 00000 00000" className="school-input" data-testid="input-phone" /></label><label className="grid gap-2 text-xs font-bold">Email address<input required type="email" value={form.email} onChange={update('email')} placeholder="you@example.com" className="school-input" data-testid="input-email" /></label></div><label className="grid gap-2 text-xs font-bold">Grade applying for<select required value={form.className} onChange={update('className')} className="school-input" data-testid="select-class"><option value="">Choose a grade</option><option>Pre-Primary</option><option>Class 1–4</option><option>Class 5–10</option></select></label><label className="grid gap-2 text-xs font-bold">Message<textarea value={form.message} onChange={update('message')} rows={4} placeholder="Tell us how we can help..." className="school-input resize-none" data-testid="textarea-message" /></label><button type="submit" className="inline-flex w-fit items-center gap-2 rounded-full bg-[#d95340] px-5 py-3.5 text-sm font-extrabold text-[#fff8ee] transition-transform hover:-translate-y-0.5" data-testid="button-submit-enquiry">Email enquiry <Send size={15} /></button><p className="text-[11px] text-[#202337]/45">Your details stay in your email app until you choose to send them to the school office.</p></form>}</div></div></section>
  </>;
}
...
function Contact() {
  ...
  return <>
    ...
    <section className="bg-[#e3b45b] px-5 py-16"><div className="page-wrap flex flex-col justify-between gap-7 md:flex-row md:items-center"><div><p className="font-mono-school text-[10px] uppercase tracking-[.16em] text-[#202337]/60">Stay in the loop</p><h2 className="mt-3 font-display text-4xl font-bold text-[#202337]">Follow our school days.</h2></div><div className="flex flex-wrap gap-3"><a href="https://www.instagram.com/saraswatischool2016?igsi=N2szeHhuN214Z3Rp" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#202337] px-5 py-3 text-sm font-bold text-[#f8eedc]" data-testid="link-contact-instagram"><Instagram size={16} /> Instagram</a><a href="https://www.facebook.com/share/1EigafJA4U/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#202337]/25 px-5 py-3 text-sm font-bold text-[#202337]" data-testid="link-contact-facebook">Facebook</a><a href="tel:+912572211814" className="inline-flex items-center gap-2 rounded-full border border-[#202337]/25 px-5 py-3 text-sm font-bold text-[#202337]" data-testid="link-contact-call"><Phone size={15} /> Call now</a></div></div></section>
  </>;
}
