...
function DocumentLink({ document, index, section }: { document: { title: string; file: string; files?: readonly { file: string; label: string }[] }; index: number; section: string }) {
  const files = document.files?.length ? document.files : document.file ? [{ file: document.file, label: 'View PDF' }] : [];
  return files.length ? <div className="flex flex-col items-start gap-2">{files.map((file, fileIndex) => <span key={file.file} className="flex flex-wrap gap-x-4 gap-y-1"><a href={file.file} target="_blank" rel="noopener noreferrer" className="font-bold uppercase text-[#3b5794] underline decoration-1 underline-offset-2 hover:text-[#c94b35]" data-testid={`link-disclosure-pdf-${section}-${index}-${fileIndex}`}>[{file.label}]</a><a href={file.file} download className="font-bold uppercase text-[#3b5794] underline decoration-1 underline-offset-2 hover:text-[#c94b35]" data-testid={`download-disclosure-pdf-${section}-${index}-${fileIndex}`}>[Download]</a></span>)}</div> : <span className="font-bold uppercase text-[#202337]/40" data-testid={`status-disclosure-pdf-${section}-${index}`}>[PDF pending]</span>;
}
