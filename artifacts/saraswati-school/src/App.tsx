...
function MandatoryDisclosure() {
  return <>
     ...
     <section className="bg-[#f4e6c9] px-5 py-20 md:py-28"><div className="page-wrap"><SectionHeading eyebrow="B · Documents and information" title="School records, ready to view." copy="Add each PDF to public/documents and set its file path in src/siteContent.ts. The uploaded document link will appear automatically in the table." /><div className="mt-12 grid gap-12">{schoolContent.mandatoryDisclosure.documentSections.map(section => <DisclosureDocumentSection key={section.code} section={section} />)}</div></div></section>
...
function DisclosureDocumentSection({ section }: { section: (typeof schoolContent.mandatoryDisclosure.documentSections)[number] }) {
  return <section aria-labelledby={`document-section-${section.code.toLowerCase()}`}><h2 id={`document-section-${section.code.toLowerCase()}`} className="mb-5 rounded-xl bg-[#202337] px-5 py-4 font-mono-school text-sm font-bold uppercase tracking-[.08em] text-[#f8eedc] sm:text-base">{section.code}: {section.title}</h2><div className="overflow-hidden rounded-[1.7rem] border border-[#202337]/12 bg-[#f8eedc]"><div className="hidden overflow-x-auto md:block"><table className="w-full border-collapse text-left"><thead><tr className="border-b border-[#202337]/15 bg-[#202337]/5 font-mono-school text-[10px] uppercase tracking-[.1em] text-[#202337]/70"><th className="w-20 px-5 py-4">SL No.</th><th className="px-5 py-4">Documents / Information</th><th className="w-64 px-5 py-4">Uploaded Document Link (PDF)</th></tr></thead><tbody>{section.documents.map((document, index) => <tr key={document.title} className="border-b border-[#202337]/10 last:border-0"><td className="px-5 py-5 align-top font-mono-school text-xs font-bold text-[#c77a22]">{index + 1}</td><th scope="row" className="px-5 py-5 text-sm font-semibold leading-6 text-[#202337]">{document.title}</th><td className="px-5 py-5 align-top"><DocumentLink document={document} index={index} section={section.code} /></td></tr>)}</tbody></table></div><div className="grid gap-3 p-3 md:hidden">{section.documents.map((document, index) => <div key={document.title} className="rounded-2xl border border-[#202337]/10 bg-[#f4e6c9] p-4"><div className="flex items-start gap-3"><span className="font-mono-school text-xs font-bold text-[#c77a22]">{String(index + 1).padStart(2, '0')}</span><p className="min-w-0 flex-1 text-sm font-bold leading-5 text-[#202337]">{document.title}</p></div><div className="mt-3 border-t border-[#202337]/10 pt-3"><p className="mb-1 font-mono-school text-[9px] font-bold uppercase tracking-[.1em] text-[#c77a22]">Uploaded document link (PDF)</p><DocumentLink document={document} index={index} section={section.code} /></div></div>)}</div></div></section>;
}
